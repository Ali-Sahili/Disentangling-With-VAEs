...
def train_WAE_MMD(args, data_loader, device):

    model = WAE_MMD(args.nc, args.z_dim, args.reg_weight, args.kernel_type,
                         args.latent_var).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        for (data, label) in data_loader:
            ...
            losses = model.loss_function(x, x_recon, z)
 
            kl_loss_t += losses["MMD"].item()
            ...
            if global_iter % args.display_step == 0:
                pbar.write('[{}] recon_loss:{:.3f} | MMD:{:.3f}'.format(
